...
fn main(@builtin(position) frag_coord: vec4<f32>) -> @location(0) vec4<f32> {
  ...
  let ring1 = smoothstep(pulse, pulse - 0.012, r);
  let ring2 = smoothstep(0.52 + 0.08 * sin(t * 1.2), 0.50 + 0.08 * sin(t * 1.2), r);
  let ring3 = smoothstep(0.76 + 0.06 * sin(t * 0.7), 0.74 + 0.06 * sin(t * 0.7), r);
